...
#ifdef Q_OS_WIN
  return m_windowsAudio->isDeviceActive(connectedDeviceMacAddress);
#else
...
void MediaController::handleConversationalAwareness(const QByteArray &data) {
...
#ifdef Q_OS_WIN
      initialVolume = m_windowsAudio->getVolume();
#else
...
#ifdef Q_OS_WIN
      m_windowsAudio->setVolume(target);
#else
      QString sink = m_pulseAudio->getDefaultSink();
  ...
  default:
    ...
    if (initialVolume != -1 && isActiveOutputDeviceAirPods()) {
#ifdef Q_OS_WIN
      m_windowsAudio->setVolume(initialVolume);
#else
      QString sink = m_pulseAudio->getDefaultSink();
...
bool MediaController::isA2dpProfileAvailable() {
  ...
  return m_pulseAudio->isProfileAvailable(m_deviceOutputName,
                                          "a2dp-sink-sbc_xq") ||
         m_pulseAudio->isProfileAvailable(m_deviceOutputName,
                                          "a2dp-sink-sbc") ||
...
bool MediaController::restartWirePlumber() {
  ...
  int result = QProcess::execute(
      "systemctl", QStringList() << "--user" << "restart" << "wireplumber");
  if (result == 0) {
...
#ifdef Q_OS_WIN
  // Windows handles profile switching internally usually
  return;
#else
  if (connectedDeviceMacAddress.isEmpty() || m_deviceOutputName.isEmpty()) {
    LOG_WARN("Connected device MAC address or output name is empty, cannot "
...
MediaController::mediaStateFromPlayerctlOutput(const QString &output) const {
  if (output == "Playing") {
...
#ifdef Q_OS_WIN
  return MediaState::Stopped; // Placeholder for Windows SMTC
#else
  return mediaStateFromPlayerctlOutput(
      PlayerStatusWatcher::getCurrentPlaybackStatus(""));
#endif
}
...
void MediaController::pause() {
  ...
  for (const QString &service : services) {
    ...
    QVariant playbackStatus = playerInterface.property("PlaybackStatus");
    LOG_DEBUG("PlaybackStatus for " << service << ": "
...
QString MediaController::getAudioDeviceName() {
  ...
  QString cardName =
      m_pulseAudio->getCardNameForDevice(connectedDeviceMacAddress);
  if (cardName.isEmpty()) {
    LOG_ERROR("No matching Bluetooth card found for MAC address: "
