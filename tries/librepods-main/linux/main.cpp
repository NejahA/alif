...
public:
...
#endif
  {
    ...
    connect(trayManager, &TrayIconManager::trayClicked, this,
            &AirPodsTrayApp::onTrayIconActivated);
    connect(trayManager, &TrayIconManager::openApp, this,
            &AirPodsTrayApp::onOpenApp);
    connect(trayManager, &TrayIconManager::openSettings, this,
            &AirPodsTrayApp::onOpenSettings);
    connect(trayManager, &TrayIconManager::noiseControlChanged, this,
            &AirPodsTrayApp::setNoiseControlMode);
    connect(trayManager, &TrayIconManager::conversationalAwarenessToggled, this,
            &AirPodsTrayApp::setConversationalAwareness);
    connect(m_deviceInfo, &DeviceInfo::batteryStatusChanged, trayManager,
            &TrayIconManager::updateBatteryStatus);
    connect(m_deviceInfo, &DeviceInfo::noiseControlModeChanged, trayManager,
            &TrayIconManager::updateNoiseControlState);
    connect(m_deviceInfo, &DeviceInfo::conversationalAwarenessChanged,
            trayManager, &TrayIconManager::updateConversationalAwareness);
    connect(trayManager, &TrayIconManager::notificationsEnabledChanged, this,
            &AirPodsTrayApp::saveNotificationsEnabled);
    connect(trayManager, &TrayIconManager::notificationsEnabledChanged, this,
            &AirPodsTrayApp::notificationsEnabledChanged);

    // Initialize MediaController and connect signals
    mediaController = new MediaController(this);
    connect(mediaController, &MediaController::mediaStateChanged, this,
            &AirPodsTrayApp::handleMediaStateChange);
    mediaController->followMediaChanges();

#ifdef Q_OS_WIN
    m_windowsMonitor = new WindowsBluetoothMonitor(this);
    connect(m_windowsMonitor, &WindowsBluetoothMonitor::deviceConnected, this,
            &AirPodsTrayApp::bluezDeviceConnected);
    connect(m_windowsMonitor, &WindowsBluetoothMonitor::deviceDisconnected,
            this, &AirPodsTrayApp::bluezDeviceDisconnected);
    m_windowsMonitor->startMonitoring();
#else
    monitor = new BluetoothMonitor(this);
    connect(monitor, &BluetoothMonitor::deviceConnected, this,
            &AirPodsTrayApp::bluezDeviceConnected);
    connect(monitor, &BluetoothMonitor::deviceDisconnected, this,
            &AirPodsTrayApp::bluezDeviceDisconnected);
    monitor->checkAlreadyConnectedDevices();
#endif

    connect(m_bleManager, &BleManager::deviceFound, this,
            &AirPodsTrayApp::bleDeviceFound);
    connect(m_deviceInfo->getBattery(), &Battery::primaryChanged, this,
            &AirPodsTrayApp::primaryChanged);

#ifndef Q_OS_WIN
    connect(m_systemSleepMonitor, &SystemSleepMonitor::systemGoingToSleep, this,
            &AirPodsTrayApp::onSystemGoingToSleep);
    connect(m_systemSleepMonitor, &SystemSleepMonitor::systemWakingUp, this,
            &AirPodsTrayApp::onSystemWakingUp);
#endif
  ...
  const QList<QBluetoothAddress> connectedDevices =
  ...
  for (const QBluetoothAddress &address : connectedDevices) {
    ...
    if (isAirPodsDevice(device)) {
      ...
      // On startup after reboot, activate A2DP profile for already connected
      // AirPods
      QTimer::singleShot(2000, this, [this, address]() {
        ...
        LOG_INFO(
            "A2DP profile activation attempted for AirPods found on startup");
      });
      return;
    }
  }
...
bool notificationsEnabled() const {
  return trayManager->notificationsEnabled();
}
...
#ifdef Q_OS_WIN
WindowsBluetoothMonitor *m_windowsMonitor = nullptr;
#else
...
AutoStartManager *m_autoStartManager = nullptr;
...
void notifyAndroidDevice() {
  ...
  if (phoneSocket && phoneSocket->isOpen()) {
    ...
    LOG_DEBUG("Sent notification packet to Android: "
              << AirPodsPackets::Phone::NOTIFICATION.toHex());
  } else {
...
void setNoiseControlMode(NoiseControlMode mode) {
  if (m_deviceInfo->noiseControlMode() == mode) {
    LOG_INFO(
        "Noise control mode is already set to: " << static_cast<int>(mode));
    return;
  }
...
void setConversationalAwareness(bool enabled) {
  LOG_INFO("Setting conversational awareness to: " << (enabled ? "enabled"
                                                               : "disabled"));
  QByteArray packet = enabled
                          ? AirPodsPackets::ConversationalAwareness::ENABLED
...
void setOneBudANCMode(bool enabled) {
  if (m_deviceInfo->oneBudANCMode() == enabled) {
    LOG_INFO("One Bud ANC mode is already "
             << (enabled ? "enabled" : "disabled"));
    return;
  }

  LOG_INFO(
      "Setting One Bud ANC mode to: " << (enabled ? "enabled" : "disabled"));
...
void setAdaptiveNoiseLevel(int level) {
  ...
  if (m_deviceInfo->adaptiveNoiseLevel() != level &&
      m_deviceInfo->adaptiveModeActive()) {
...
void setEarDetectionBehavior(int behavior) {
  ...
  mediaController->setEarDetectionBehavior(
...
void setCrossDeviceEnabled(bool enabled) {
  if (CrossDevice.isEnabled == enabled) {
    LOG_INFO("Cross-device feature is already "
             << (enabled ? "enabled" : "disabled"));
    return;
  }
...
void setPhoneMac(const QString &mac) {
  ...
  // Basic MAC address validation (accepts formats like AA:BB:CC:DD:EE:FF,
...
void saveCrossDeviceEnabled() {
  m_settings->setValue("crossdevice/enabled", CrossDevice.isEnabled);
}

int loadEarDetectionSettings() {
  return m_settings
      ->value("earDetection/setting",
...
void saveEarDetectionSettings() {
  m_settings->setValue("earDetection/setting",
                       mediaController->getEarDetectionBehavior());
}
...
void onSystemWakingUp() {
  ...
  if (areAirpodsConnected() && m_deviceInfo &&
    ...
    LOG_INFO(
        "AirPods already connected after wake-up, re-activating A2DP profile");
    mediaController->setConnectedDeviceMacAddress(
        m_deviceInfo->bluetoothAddress().replace(":", "_"));

    // Always activate A2DP profile after system wake since the profile might
    // have been lost
...
void onOpenSettings() {
  QObject *rootObject = parent->rootObjects().first();
  if (rootObject) {
    QMetaObject::invokeMethod(rootObject, "reopen",
...
void bluezDeviceConnected(const QString &address, const QString &name) {
  ...
  // Attempt to activate A2DP profile after a delay to ensure connection is
  // established
  QTimer::singleShot(2000, this, [this, address]() {
    if (!address.isEmpty()) {
...
void onDeviceDisconnected(const QBluetoothAddress &address) {
  ...
  if (phoneSocket && phoneSocket->isOpen()) {
    ...
    LOG_DEBUG("AIRPODS_DISCONNECTED packet written: "
              << AirPodsPackets::Connection::AIRPODS_DISCONNECTED.toHex());
  }
  ...
  trayManager->showNotification(tr("AirPods Disconnected"),
...
void parseMetadata(const QByteArray &data) {
  ...
  // Check if there is enough data to skip the initial bytes (based on example
  // structure)
  if (data.size() < pos + 6) {
...
QString getEarStatus(char value) {
  return (value == 0x00)   ? "In Ear"
         : (value == 0x01) ? "Out of Ear"
                           : "In case";
}
...
void connectToDevice(const QBluetoothDeviceInfo &device) {
  ...
  QBluetoothSocket *localSocket =
      new QBluetoothSocket(QBluetoothServiceInfo::L2capProtocol);
  socket = localSocket;

  // Connection handler
  auto handleConnection = [this, localSocket]() {
    connect(localSocket, &QBluetoothSocket::readyRead, this,
            [this, localSocket]() {
              QByteArray data = localSocket->readAll();
              QMetaObject::invokeMethod(this, "parseData", Qt::QueuedConnection,
                                        Q_ARG(QByteArray, data));
              QMetaObject::invokeMethod(this, "relayPacketToPhone",
                                        Qt::QueuedConnection,
                                        Q_ARG(QByteArray, data));
            });
    sendHandshake();
  ...
  auto handleError = [this, device,
                      localSocket](QBluetoothSocket::SocketError error) {
  ...
  localSocket->connectToService(
      device.address(), QBluetoothUuid("74ec2172-0bad-4d01-8f77-997b2be0722a"));
...
void parseData(const QByteArray &data) {
  ...
  // Magic Cloud Keys Response
               ...
               AirPodsPackets::MagicPairing::MAGIC_CLOUD_KEYS_HEADER)) {
  ...
  // Get CA state
    ...
    if (auto result =
            AirPodsPackets::ConversationalAwareness::parseState(data)) {
      m_deviceInfo->setConversationalAwareness(result.value());
      LOG_INFO("Conversational awareness state received: "
               << m_deviceInfo->conversationalAwareness());
    }
  }
  // Hearing Aid state
    ...
    if (auto result = AirPodsPackets::HearingAid::parseState(data)) {
      ...
      LOG_INFO(
          "Hearing aid state received: " << m_deviceInfo->hearingAidEnabled());
    }
  }
  // Noise Control Mode
    ...
    if (auto value = AirPodsPackets::NoiseControl::parseMode(data)) {
      ...
      LOG_INFO(
          "Noise control mode received: " << m_deviceInfo->noiseControlMode());
    }
  }
  // Ear Detection
  else if (data.size() == 8 &&
  ...
  // Conversational Awareness Data
           ...
           data.startsWith(
               AirPodsPackets::ConversationalAwareness::DATA_HEADER)) {
    ...
    mediaController->setConnectedDeviceMacAddress(
        m_deviceInfo->bluetoothAddress().replace(":", "_"));
    if (m_deviceInfo->getEarDetection()
            ->oneOrMorePodsInEar()) // AirPods get added as output device only
                                    // after this
    {
      mediaController->activateA2dpProfile();
    }
...
void connectToPhone() {
  ...
  QBluetoothAddress phoneAddress(
      "00:00:00:00:00:00"); // Default address, will be overwritten if
                            // PHONE_MAC_ADDRESS is set
  ...
  connect(phoneSocket, &QBluetoothSocket::connected, this, [this]() {
    ...
    if (!lastBatteryStatus.isEmpty()) {
      ...
      LOG_DEBUG(
          "Sent last battery status to phone: " << lastBatteryStatus.toHex());
    }
    if (!lastEarDetectionStatus.isEmpty()) {
      phoneSocket->write(lastEarDetectionStatus);
      LOG_DEBUG("Sent last ear detection status to phone: "
                << lastEarDetectionStatus.toHex());
    }
  });
  ...
  connect(phoneSocket,
          ...
          this, [this](QBluetoothSocket::SocketError error) {
            LOG_ERROR("Phone socket error: " << error << ", "
                                             << phoneSocket->errorString());
          });

  phoneSocket->connectToService(
...
void handlePhonePacket(const QByteArray &packet) {
  if (packet.startsWith(AirPodsPackets::Phone::NOTIFICATION)) {
    ...
    QByteArray response = (socket && socket->isOpen())
    ...
    if (socket && socket->isOpen()) {
      ...
      QProcess process;
      process.start("bluetoothctl", QStringList()
                                        << "disconnect"
...
void onPhoneDataReceived() {
  ...
  QMetaObject::invokeMethod(this, "handlePhonePacket", Qt::QueuedConnection,
                            Q_ARG(QByteArray, data));
}
...
void bleDeviceFound(const BleInfo &device) {
  if (BLEUtils::isValidIrkRpa(m_deviceInfo->magicAccIRK(), device.address)) {
    ...
    auto decryptet = BLEUtils::decryptLastBytes(device.encryptedPayload,
                                                m_deviceInfo->magicAccEncKey());
    m_deviceInfo->getBattery()->parseEncryptedPacket(
        decryptet, device.primaryLeft, device.isThisPodInTheCase,
...
void handleMediaStateChange(MediaController::MediaState state) {
  if (state == MediaController::MediaState::Playing) {
    LOG_INFO("Media started playing, sending disconnect request to Android and "
             "taking over audio");
...
void sendDisconnectRequestToAndroid() {
  ...
  if (phoneSocket && phoneSocket->isOpen()) {
    ...
    LOG_DEBUG("Sent disconnect request to Android: "
              << AirPodsPackets::Phone::DISCONNECT_REQUEST.toHex());
  } else {
    LOG_WARN("Phone socket is not open, cannot send disconnect request");
  }
}

bool isPhoneConnected() { return phoneSocket && phoneSocket->isOpen(); }
...
void connectToAirPods(bool force) {
  ...
  for (const QBluetoothAddress &address : connectedDevices) {
    QBluetoothDeviceInfo device(address, "", 0);
    LOG_DEBUG("Connected device: " << device.name() << " ("
...
int main(int argc, char *argv[]) {
  ...
  // Expose PHONE_MAC_ADDRESS environment variable to QML for placeholder in
  // settings
  {
    QProcessEnvironment env = QProcessEnvironment::systemEnvironment();
    ...
    trayApp->updatePhoneMacStatus(phoneMacEnv.isEmpty()
                                      ? QStringLiteral("No phone MAC set")
                                      : phoneMacEnv);
  }
  ...
  QObject::connect(&server, &QLocalServer::newConnection, [&]() {
    ...
    QObject::connect(socket, &QLocalSocket::readyRead,
                     [socket, &engine, &trayApp]() {
                       QString msg = socket->readAll();
                       // Check if the message is "reopen", if so, trigger
                       // onOpenApp function
                       if (msg == "reopen") {
