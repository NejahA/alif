...
let favorites: WikiSummary[] = JSON.parse(localStorage.getItem('313rifts-favorites') || '[]');
let history: WikiSummary[] = JSON.parse(localStorage.getItem('313rifts-history') || '[]');
...
async function fetchRandomArticles(count: number) {
  ...
  try {
    ...
    if (!randomRes.ok) {
      throw new Error(`HTTP ${randomRes.status}`);
    }
    ...
    showToast('Failed to load articles. Please check your connection.', 'error');
...
function toggleTheme() {
  ...
  localStorage.setItem('313rifts-theme', isLight ? 'light' : 'dark');
}

function applyTheme() {
  const savedTheme = localStorage.getItem('313rifts-theme');
  if (savedTheme === 'light') {
...
function toggleFavorite(article: WikiSummary) {
  ...
  localStorage.setItem('313rifts-favorites', JSON.stringify(favorites));
  loadFavorites();
}

function loadFavorites() {
  favorites = JSON.parse(localStorage.getItem('313rifts-favorites') || '[]');
}
...
function addToHistory(article: WikiSummary) {
  ...
  localStorage.setItem('313rifts-history', JSON.stringify(history));
  loadHistory();
}

function loadHistory() {
  history = JSON.parse(localStorage.getItem('313rifts-history') || '[]');
}
...
function setupKeyboardShortcuts() {
  document.addEventListener('keydown', (e) => {
    ...
    // Escape key to clear search or go back
    if (e.key === 'Escape') {
      if (searchInput.value.trim()) {
        searchInput.value = '';
        fetchRandomArticles(6);
      } else if (!landingView.classList.contains('hidden')) {
        // Already on landing view
      } else {
        showLandingView();
      }
      return;
    ...
    // Ctrl+D for dark/light theme toggle
