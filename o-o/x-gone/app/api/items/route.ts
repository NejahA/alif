...
export async function GET() {
  try {
    ...
    const itemsWithNumbers = items.map((item: any) => ({
      ...
      expiresAt: item.expiresAt instanceof Date ? item.expiresAt.getTime() : new Date(item.expiresAt).getTime(),
    }));

    return NextResponse.json({ success: true, data: itemsWithNumbers });
  } catch (error) {
    console.error("GET Error:", error);
    return NextResponse.json({ success: false, error: 'Failed to fetch items', details: (error as Error).message }, { status: 500 });
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ success: false, error: 'Failed to create item', details: (error as Error).message }, { status: 400 });
  }
}
