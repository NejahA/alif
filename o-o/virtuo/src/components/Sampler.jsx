...
export default function Sampler() {
  ...
  useEffect(() => {
    // We'll use a synth to simulate samples for this task
    // since we don't have external audio files easily accessible.
    // In a real app, this would be Tone.Sampler with URLs.
  ...
  const triggerSample = async (note, type) => {
    await Tone.start();
    
    // Shift note by semitones
    const shiftedNote = Tone.Frequency(note).transpose(pitch).toNote();
    ...
    samplerRef.current.triggerAttackRelease(shiftedNote, '1n');
  };
