...
export default function Guitar() {
  ...
  const strum = async () => {
    await Tone.start();
    const chordFrets = CHORDS[selectedChord];
    
    // Play strings with slight delay
    STRINGS.forEach((_, i) => {
      const fret = chordFrets[i];
      if (fret !== -1) {
        const note = getNote(i, fret);
        setTimeout(() => playNote(note), i * 50);
      }
    });
  };
  ...
  return (
    <div style={{ width: '100%', overflowX: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div className="glass-panel" style={{ padding: '15px', display: 'flex', justifyContent: 'center', gap: '15px', alignItems: 'center' }}>
        <div style={{ display: 'flex', gap: '8px' }}>
          {Object.keys(CHORDS).map(chord => (
            <button
              key={chord}
              className={`btn-glass ${selectedChord === chord ? 'active' : ''}`}
        ...
        <button 
          ...
          onClick={strum}
          style={{ padding: '8px 20px', background: 'var(--accent-primary)', borderColor: 'white' }}
        >
          STRUM CHORD
        </button>
      </div>
