...
function App() {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        alert('Backend server not running. Please start the backend server on port 5001.');
        setLoading(false);
      }
    };
  ...
  const addTask = async (task: Omit<Task, '_id' | 'createdAt' | 'updatedAt'>) => {
    try {
      ...
      alert('Failed to add task. Please check backend connection.');
    }
  };

  const moveTask = async (id: string, newStatus: Status) => {
    try {
      const updatedTask = await updateTask(id, { status: newStatus, updatedAt: new Date() });
      setTasks(tasks.map(t => t._id === id ? updatedTask : t));
    } catch (error) {
      console.error('Failed to move task:', error);
      alert('Failed to update task. Please check backend connection.');
    }
  };
  ...
  const deleteTask = async (id: string) => {
    try {
      ...
      alert('Failed to delete task. Please check backend connection.');
    }
  };

  const handleImport = (importedTasks: Task[]) => {
    // This would need to be implemented with bulk API calls
    alert('Import functionality would need backend implementation');
  };
  ...
  const handleAISuggestion = async (suggestion: Partial<Task>) => {
    try {
      const newTask = await createTask({
        ...suggestion as Task,
        status: 'todo',
  ...
  const handleApplyTemplate = (templateTasks: Omit<Task, '_id' | 'createdAt' | 'updatedAt'>[]) => {
    // This would need to be implemented with bulk API calls
    alert('Template application would need backend implementation');
  };

  const handleSync = async () => {
    try {
      // Reload all data from backend
      const [tasksData, usersData, commentsData] = await Promise.all([
        fetchTasks(),
        fetchUsers(),
        fetchComments()
      ]);
      ...
      alert('Failed to sync. Please check backend connection.');
    }
  };

  const handleRetry = () => {
    // In a real app, this would retry failed syncs
    alert('Retrying sync...');
  };
  ...
  const handleAssignTask = async (taskId: string, userId: string) => {
    try {
      const updatedTask = await updateTask(taskId, { assigneeId: userId, updatedAt: new Date() });
      setTasks(tasks.map(task => 
        task._id === taskId ? updatedTask : task
      ));
    } catch (error) {
      console.error('Failed to assign task:', error);
      alert('Failed to assign task. Please check backend connection.');
    }
  };
  ...
  const handleAddComment = async (taskId: string, content: string) => {
    ...
    try {
      ...
      alert('Failed to add comment. Please check backend connection.');
    }
  };
  ...
  const handleDeleteComment = async (commentId: string) => {
    try {
      ...
      alert('Failed to delete comment. Please check backend connection.');
    }
  };

  // Integration functions
  const handleToggleIntegration = async (integrationId: string, connected: boolean) => {
    try {
      const updatedIntegration = await updateIntegration(integrationId, { connected });
  ...
  const handleCreateWorkflow = async (workflow: any) => {
    try {
      ...
      alert('Failed to create workflow. Please check backend connection.');
    }
  };
  ...
  const handleVerifyTask = async (taskId: string) => {
    try {
      const verification = await createBlockchainVerification({
        taskId,
      ...
      setBlockchainVerifications([...blockchainVerifications, verification]);
    } catch (error) {
      console.error('Failed to verify task:', error);
      alert('Failed to verify task on blockchain. Please check backend connection.');
    }
  };
          ...
          <button 
            ...
            onClick={async () => {
              try {
                ...
                alert('Failed to seed database. Make sure backend is running on port 5000.');
              }
            }}
          >
        ...
        <div className="content-area">
          ...
          {activeView === 'search' && (
            <AdvancedSearch 
              ...
              onSearchResults={(results) => {
                // In a real app, this would update filtered tasks
          ...
          {activeView === 'scheduler' && (
            <AutoScheduler 
              ...
              onScheduleTasks={(scheduledTasks) => {
                // Update tasks with scheduled dates
          ...
          {activeView === 'blockchain' && (
            <BlockchainVerification 
              onTaskVerify={handleVerifyTask}
              onAuditRequest={() => alert('Audit request submitted')}
              onSmartContractDeploy={(contract) => alert(`Smart contract deployed: ${contract.name}`)}
            />
          )}
              ...
              {loading ? (
                ...
                <div className="comments-grid">
                  {tasks.map(task => {
                    ...
                    const taskUser = teamMembers.find(user => user._id === task.assigneeId) || user;
                    
                    return (
                      <div key={task._id} className="task-comments-card">
                          ...
                          {taskComments.length === 0 ? (
                            ...
                            taskComments.map(comment => {
                              const commentUser = typeof comment.userId === 'object' 
                                ? comment.userId 
                                : teamMembers.find(user => user._id === comment.userId) || user;
                              return (
                                <div key={comment._id} className="comment-item">
                                  <div className="comment-header">
                                    ...
                                    {user && typeof comment.userId === 'string' && comment.userId === user._id && (
                                      <button 
                          ...
                          <textarea 
                            ...
                            onKeyDown={(e) => {
                              if (e.key === 'Enter' && e.ctrlKey) {
                                ...
                                if (content && user) {
                                  handleAddComment(task._id, content);
